...
void maximalClique::hybrid_pivot_enumerate(int *RP, int *R, double q, int rsize, node *Can, int csize, list &X, int xsize, ofstream &out)
{
	assert(RP[0] == 1);
	iterations++;
	...
	for (int i = 0; i < qsize; ++i) {
		...
		hybrid_pivot_enumerate(RPN, R, qn, rsize+1, CanN, cnsize, Xn, xnsize, out);
...
{
	ofstream out("enron_test.txt",ios::out);
	...
	int tep=0;
    tep=core_decompsition(nodeset,nodesize);
	cout<<"core number:  "<<tep<<endl;

	vector<bool> visited(V, false);
	initSubgrah();
	// int R[maxclmums], RP[maxclmums];
	int R[maxdeg+1], RP[maxdeg+1];
	int csize = 0, xsize = 0;
	...
	for (int i = 0; i < nodesize; ++i) {
		...
		R[0] = u; RP[0] = 1; RP[1] = u; 
		if (csize > 0 ) hybrid_pivot_enumerate(RP, R, 1.0f, 1, Can, csize, X, xsize, out);
	}
