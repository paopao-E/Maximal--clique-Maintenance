...
void Include_Edge_Clique::jiabian(G &g, int u, int v, float q,int &change_size)
{
    vector<vector<int> > clique;
    vector<vector<int> > Del;
    vector<vector<int> > New;
    ...
    for (int i = 0; i < New.size(); i++)
    {
        ...
        for (int j = 0; j < new_clique.size(); j++)
        {
            ...
            for (int m = 0; m < new_clique[j].size(); m++)
            {
                ...
                for (int k = 0; k < g.adj[new_clique[j][m]].vec.size(); k++)
                {
                    // if(g.adj[new_clique[j][m]].vec[k].vertex>=0)
                    a.push_back(g.adj[new_clique[j][m]].vec[k].vertex);
                }
                sort(a.begin(),a.end());
                mxed.push_back(a);
            ...
            {
                bool tep1=true;
                for(int t=0;t<g.adj[new_clique[j][0]].vec.size();t++)
                {
                    if(FindValue(new_clique[j][0],g.adj[new_clique[j][0]].vec[t].vertex,g)>=eta) {tep1=false;break;}
                }
                if(tep1) Del.push_back(new_clique[j]);
            }
            else{
                vector<int> mx;
                set_intersection(mxed[0].begin(),mxed[0].end(),mxed[1].begin(),mxed[1].end(),back_inserter(mx));
                ...
                if(mx.size()==0) Del.push_back(new_clique[j]);
                else
                {
                    float b=1;
                    for(int i=0;i<new_clique[j].size();i++)
...
void Include_Edge_Clique::gailvzengjia( G &g, int u, int v, float q,int &change_size)
{
    vector<vector<int> > S;
    vector<vector<int> > F;
    vector<int>Q;
...
void Include_Edge_Clique::gailvjianxiao(G &g, int u, int v, float q,int &change_size)
{
    ...
    for(int i=0;i<New.size();i++)
   {
    ...
    // vector<Vertex_Value> X;
    // vector<vector<int> > clique;
    // vector<vector<int> > clique1;
    // vector<vector<int> > clique2;
    // vector<int> R;
    // vector<int> P;  
    // set<int>new_vertex;
    // begin2_clock=clock();
    ...
    //     {
    //         Del.push_back(S[i]);
    //         for(int d=0;d<S[i].size();d++)
    //         {
    //             new_vertex.insert(S[i][d]);
    //         }
    //     }
    ...
    //     temp_g.adj=new node[g.vernum];
    //     temp_g.deg=new int[g.vernum];
    //     for(int i=0;i<Del.size();i++)
    //     {
    //         for(int j=0;j<Del[i].size();j++)
    //         {
    //             temp_g.adj[Del[i][j]].vertex=Del[i][j];
    //             for(int t=j+1;t<Del[i].size();t++)
    //             {
    //                 Vertex_Value temp;
    //                 temp.vertex=Del[i][t];
                  
    //                 float r=FindValue(Del[i][j],Del[i][t],g);
    //                 temp.value=r;
    //                 temp_g.adj[Del[i][j]].vec.push_back(temp);
    //                 temp_g.deg[Del[i][j]]++;

    //                 Vertex_Value temp1;
    //                 temp.vertex=Del[i][j];
    //                 temp.value=r;
    //                 temp_g.adj[Del[i][t]].vec.push_back(temp);
    //                 temp_g.deg[Del[i][t]]++;
    //             }
                
    //         }
    //     }
    //     // for (int i = 0; i < temp_g.adj[u].vec.size(); i++)
    //     // {
    //     //     if (temp_g.adj[u].vec[i].vertex == v)
    //     //         temp_g.adj[u].vec[i].value = q;
    //     // }
    //     // for (int i = 0; i < temp_g.adj[v].vec.size(); i++)
    //     // {
    //     //     if (temp_g.adj[v].vec[i].vertex == u)
    //     //         temp_g.adj[v].vec[i].value = q;
    //     // }

    //     maximaletaclique(R, 1, C, X, P, eta, temp_g, clique);
    //     if(clique.size()!=0)
    //     {
    //         for (int j = 0; j < clique.size(); j++)
    //         {
...
void Include_Edge_Clique::print_information()
{
    ...
    cout<<"new time:  "<<elp1_clock<<endl;
    cout<<"del time:  "<<elp2_clock<<endl;
}
...
{
    char *argv2[] = {"DUclique", "./test.txt", "./test_1.txt", "0.6"};
    G g;
    float eta = 0, q = 0;
    int u = 0, v = 0;
    ...
    // struct timespec beg_clock, end_clock, elapsed_time;
    ...
    for (int i = 0; i < counts; i++)
    {
        ...
        {               
            ...
            iec.v = v;
            iec.q = q;
            iec.jiabian(g, u, v, q,change_size);            
        }
        if (sign == 3) 
        {
            iec.u = u;
            iec.v = v;
            iec.q = q;
            iec.gailvzengjia(g, u, v, q,change_size);
        }
        if (sign == 4) 
        {
            iec.u = u;
